...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Serve combined videos (these are generated by the video processor)
  app.get("/combined-video/:id", async (req, res) => {
    try {
      ...
      const videoFilename = `remix_${videoId}.mp4`;
      const videoPath = path.join(process.cwd(), "static", "videos", videoFilename);
      
      console.log(`Serving combined video: ${videoPath}`);
